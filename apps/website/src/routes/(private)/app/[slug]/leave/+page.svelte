...
<div class="container mx-auto space-y-6 p-4 md:p-6 lg:p-8">
  ...
  <Card class="p-6">
    <div class="text-center">
      <Button onclick={() => goto(`/app/${currentSlug}/leave/apply`)} size="lg" class="px-8 py-6 text-lg">
        Apply for Leave
      </Button>
      <p class="text-gray-600 dark:text-gray-400 mt-2">Request new leave or view pending requests</p>
    </div>
  </Card>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div>
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">{mockLeaveBalance.annual.used}/{mockLeaveBalance.annual.total}</span>
        </div>
        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
          <div class="bg-blue-600 dark:bg-blue-400 h-3 rounded-full" style="width: {Math.round((mockLeaveBalance.annual.used / mockLeaveBalance.annual.total) * 100)}%"></div>
        </div>
      </div>
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">{mockLeaveBalance.sick.used}/{mockLeaveBalance.sick.total}</span>
        </div>
        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
          <div class="bg-red-600 dark:bg-red-400 h-3 rounded-full" style="width: {Math.round((mockLeaveBalance.sick.used / mockLeaveBalance.sick.total) * 100)}%"></div>
        </div>
      </div>
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          ...
          <span class="text-sm text-gray-600 dark:text-gray-400">{mockLeaveBalance.pending} days</span>
        </div>
        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
          <div class="bg-amber-600 dark:bg-amber-400 h-3 rounded-full" style="width: 50%"></div>
        </div>
      </div>
    </div>
  </Card>
      ...
      {#each mockLeaveHistory as leave}
        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">{leave.dateRange} ({leave.days} days)</p>
